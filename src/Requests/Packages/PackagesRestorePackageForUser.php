 ...
 *   - The same package namespace and
 * version is still available and not reused for a new package. If the same package namespace is not
 * available, you will not be able to restore your package. In this scenario, to restore the deleted
 * package, you must delete the new package that uses the deleted package's namespace first.
 *
 * If the
 * `package_type` belongs to a GitHub Packages registry that supports granular permissions, the
 * authenticated user must have admin permissions to the package. For the list of these registries, see
 ...
 */
class PackagesRestorePackageForUser extends Request implements HasBody {
    use HasJsonBody;

    protected Method $method = Method::POST;

    public function resolveEndpoint(): string {
        return "/users/{$this->username}/packages/{$this->packageType}/{$this->packageName}/restore";
    }
